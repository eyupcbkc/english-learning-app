...
export default function MatchExercise({ exercises, onScore }) {
  ...
  useEffect(() => {
    if (allMatched) {
      onScore?.(exercises.length, exercises.length)
    }
  }, [allMatched])
  ...
  return (
    <Card>
      ...
      <CardContent>
        <p className="text-xs text-muted-foreground mb-4">Soldan kelimeyi, sağdan Türkçe karşılığını seç</p>
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            {exercises.map((ex, i) => (
              <button key={i} onClick={() => tryMatch('left', ex.left)}
                className={`w-full text-center border rounded-lg px-4 py-3 text-sm font-medium transition-all ${
                  matched.includes(ex.left) ? 'border-green-500 bg-green-50 text-green-700'
                    : selectedLeft === ex.left ? 'border-primary bg-primary/5 text-primary'
          ...
          <div className="space-y-2">
            {shuffledRight.map((right, i) => (
              <button key={i} onClick={() => tryMatch('right', right)}
                className={`w-full text-center border rounded-lg px-4 py-3 text-sm font-medium transition-all ${
                  matched.includes(right) ? 'border-green-500 bg-green-50 text-green-700'
                    : selectedRight === right ? 'border-primary bg-primary/5 text-primary'
                    : 'border-border hover:border-primary/50'
                }`}>{right}</button>
            ))}
          </div>
        ...
        {allMatched && (
          <div className="flex items-center gap-3 rounded-xl p-4 mt-4 bg-green-50 border border-green-200">
            ...
            <span className="text-sm font-medium">Hepsini eşleştirdin!</span>
          </div>
        )}
      </CardContent>
    </Card>
