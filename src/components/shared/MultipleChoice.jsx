...
export default function MultipleChoice({ exercises, onScore }) {
  const [answers, setAnswers] = useState({})
  const [checked, setChecked] = useState(false)

  const handleSelect = (qi, oi) => { if (!checked) setAnswers(prev => ({ ...prev, [qi]: oi })) }
  const score = exercises.reduce((acc, ex, i) => acc + (answers[i] === ex.correct ? 1 : 0), 0)

  const check = () => {
    setChecked(true)
    const s = exercises.reduce((acc, ex, i) => acc + (answers[i] === ex.correct ? 1 : 0), 0)
          ...
          <div key={qi} className="rounded-xl border p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {ex.options.map((opt, oi) => {
                ...
                return <button key={oi} className={cls} onClick={() => handleSelect(qi, oi)}>{opt}</button>
              })}
            </div>
          </div>
        ))}
